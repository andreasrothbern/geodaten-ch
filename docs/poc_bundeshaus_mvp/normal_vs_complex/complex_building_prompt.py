...
def _generate_zone_descriptions(zones: List[Dict[str, Any]]) -> str:
    ...
    for zone in zones:
        ...
        hoehe = zone.get('gebaeudehoehe_m') or zone.get('firsthoehe_m') or zone.get('building_height_m', 10)
...
def generate_complex_elevation_prompt(
    ...
    max_hoehe = 10
    for zone in zones:
        h = zone.get('gebaeudehoehe_m') or zone.get('firsthoehe_m') or zone.get('building_height_m', 0)
        if h > max_hoehe:
            max_hoehe = h
...
def generate_complex_cross_section_prompt(
    ...
    egid = building_data.get('egid', '-')
    geschosse = building_data.get('geschosse') or building_data.get('gastw') or 3
    tiefe = building_data.get('gebaeudetiefe_m') or 15
    
    # Maximale Höhe aus Zonen
    max_hoehe = 10
    for zone in zones:
        h = zone.get('gebaeudehoehe_m') or zone.get('firsthoehe_m') or 0
        if h > max_hoehe:
            max_hoehe = h
